...
<div class="row">
    <div class="flex xs12">
      <va-card >
        <va-card-title><h2>Wählen Sie ein Getränk:</h2> </va-card-title>
        <va-card-content>
...
import Auth from '@aws-amplify/auth'
    ...
    export default {
      ...
      methods:{
        add(selected){
          this.$emit('clicked', {"drink":selected.drink.S,"menu":this.menuItems[0].value.L, "icon":selected.icon + ".svg"})
        }
      },
      ...
      async mounted() {
        ...
        var config = {
            method: 'GET',
            url: that.$ConfigEndpoint+'config',
            headers: { 
          ...
          .then(function (response) {   
            ...
            console.log('menu items',that.menuItems[0].value.L);
          })
