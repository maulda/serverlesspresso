<template>
    <va-modal
      ...
      size="medium"
      title="Bestätigung"
      message="Wolle Sie die Bestellung wirklich stornieren"
      okText="Ja"
      cancelText="Nein"
    ...
    <va-card color="background" style="padding: .25rem;">
      <div class="row">
        <div class="flex md4 lg4 column-container">
          <!-- PREVIEW COLUMN -->
          <div class="display-5 mb-4">Bestellungen</div>
          <div class="row">
            <div v-for="order in previews" :key="order.orderId" class="flex md12 lg12" v-show="showButtonsOnOrder(order.baristaUserId)">
              <transition name="fade">
                <va-card stripe :stripe-color="getSeverityColor(order.age)" class="mb-4">
                  ...
                  <!-- Buttons -->
                  <div class="buttons">
                    <va-button color="danger" icon="block" :rounded="false" @click="cancelOrderClick(order)">Stornieren</va-button>
        ...
        <div class="flex md4 lg4 column-container">
          <!-- PRODUCTION COLUMN -->
          <div class="display-5 mb-4">In Bearbeitung</div>
          <div class="row">
            <div v-for="order in production" :key="order.orderId" class="flex md12 lg12">
              <transition name="fade">
                <va-card stripe :stripe-color="getSeverityColor(order.age)" class="mb-4">
                  ...
                  <!-- Buttons -->
                  <div class="buttons">
                    <va-button color="danger" icon="block" :rounded="false" @click="makeOrderClick(order.orderId, 'unmake')">Abbrechen</va-button>
                    <!-- <va-button flat :rounded="false" :disabled="true">{{ getMinsSecs (order.age) }}</va-button> -->
                    <span class="timer">{{ getMinsSecs (order.age) }}</span>
                    <va-button  color="success" icon="arrow_circle_right" :rounded="false" @click="pickupClick(order.orderId)">Fertig</va-button>
                  <!-- <va-button class="mb-2" icon="clear">Clear</va-button> -->
                  </div>
        ...
        <div class="flex md4 lg4 column-container">
          <!-- PICKUP COLUMN -->
          <div class="display-5 mb-4">Bereit Zum Abholen</div>
          <div class="row">
  ...
  methods: {
    ...
    async loadOrders () {
      ...
      try {
        try {
        // Open orders
        var { data } = await axios.get(`${this.$orderManagerEndpoint}` + `/orders?state=CREATED`, {
        ...
        data.map((order) => {
          ...
          if (order.orderNumber) {
            newOrder = {
              ...
              customerName: '',
              robot: order.robot,
              orderItemName: JSON.parse(order.drinkOrder.S).drink,
              modifiers: JSON.parse(order.drinkOrder.S).modifiers,
              startTime: order.TS,
              state: 'preview',
              age: 0
            }
      ...
      try {
        // Completed
        const { data } = await axios.get(`${this.$orderManagerEndpoint}` + `/orders?state=Completed&maxItems=10`, {
          headers: { Authorization: "Bearer " + jwtToken }
        })
        console.log('Completed orders:', data)

        data.map((order) => {
          if ((Date.now() - order.TS.N) / 1000 < MAX_AGE_LOADED_ORDERS) {
            this.orders.push({
              orderId: order.SK,
              orderNumber: order.orderNumber.S,
              customerName: '',
    ...
    async orderComplete (order) {
      ...
      try {
        const { data } = await axios({
          method: 'PUT',
          url: `${this.$orderManagerEndpoint}orders/${order.orderId}?action=complete`,
          headers: {
    ...
    async orderCancel (orderId) {
      ...
      try {
        const { data } = await axios({
          method: 'PUT',
          url: `${this.$orderManagerEndpoint}orders/${orderId}?action=cancel`,
          headers: {
            Authorization: 'Bearer ' + jwtToken
          }
        })
    ...
    async makeOrderClick (orderId, action) {
      ...
      // Refresh token // orderId.S TEST
      const session = await Auth.currentSession()
      const jwtToken = session.getAccessToken().jwtToken
      if (typeof orderId.S === "undefined"){
        try {
        const { data } = await axios({
          method: 'PUT',
          url: `${this.$orderManagerEndpoint}orders/${orderId}?action=${action}`,
          headers: {
            Authorization: 'Bearer ' + jwtToken
          }
        })
        ...
        const { data } = await axios({
          method: 'PUT',
          url: `${this.$orderManagerEndpoint}orders/${orderId.S}?action=${action}`,
          headers: {
            Authorization: 'Bearer ' + jwtToken
          }
        })
