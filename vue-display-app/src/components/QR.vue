  ...
  <div class="div tokenContainer" style="">
    ...
    <div v-show="isRefreshing && availableTokens > 0">
      <div class="mb-2" @click="toggleDebuggingInfo">
        ...
        <va-progress-bar color="#FFFFFF" :model-value="getPctProdComplete()" />
      </div>
...
export default {
  ...
  methods: {
    ...
    getQRURL () {
      return `${this.$OrderUrl}${QR_PREFIX}${this.last_code}`
    },
    getSeconds () {
    ...
    refresh () {
      ...
      // (`Reload in ${this.remaining_ms} ms`)
    ...
    async loadCode () {
      ...
      try {
        const { data } = await axios.get(this.$QRurl + `qr-code`, { headers: { Authorization: 'Bearer ' + jwtToken }})
        console.log(data)
